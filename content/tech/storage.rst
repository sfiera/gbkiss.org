...
* “CRAM space” addresses (``$0000…$7FFF``). The are 2-byte addresses within the full CRAM space (32 KiB). For example, CRAM address ``$5678`` refers to the mapped address ``$02:$B678``
...
4. Change the region’s type and type complement to ``$5A``.
...
1. Change the region’s type and type complement to ``$46``.
...
The second-to-last region of the last bank contains the file table, addressing all files. It contains a 6-byte region header, followed by 120 4-byte file table entries:
